...
type WebhookStore = {
  ...
  deleteWebhook: (url: string) => Promise<boolean>
}
...
export const useWebhookStore = create<WebhookStore>((set, get) => ({
  ...
  addWebhook: async (webhook: Webhook): Promise<boolean> => {
    const data = await window.api.webhook.get()
    const isDuplicate = data.webhooks.some(item => item.url === webhook.url)
    if (isDuplicate) {
      toast.warning('Webhook URL already exists.')
      return false
    }
  ...
  deleteWebhook: async (url: string): Promise<boolean> => {
    const data = await window.api.webhook.get()
    const updatedWebhooks = data.webhooks.filter(webhook => webhook.url !== url)
